...
graph TB
    ...
    Schedule -->|Cowork| CoworkSched[create_scheduled_task<br/><small>앱 실행 중 자동</small>]
```
...
| launchd/systemd 없음 | `manage-schedule.sh` 가드 + Cowork `create_scheduled_task` 직접 호출 (폴백: 사이드바 수동 생성) |
