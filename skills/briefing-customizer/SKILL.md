...
description: 브리핑 개인화 설정 전문가. 사용자가 브리핑 내용을 바꾸고 싶다고 하면 자동으로 활성화됩니다. 예시: "행동패턴 위주로 브리핑해줘", "캠페인 중심으로 바꿔", "이벤트 섹션 추가해줘", "이탈률 임계값 30%로", "브리핑 프리셋 변경", "브리핑 설정 보여줘"
---
...
2. 현재 `config.json`을 읽습니다 (없으면 기본값 사용)
...
| 사용자 표현 | 필드 | 변경 |
|------------|------|------|
| "이탈률 임계값 30%로" | anomaly_threshold | 30 |
