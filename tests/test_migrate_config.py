...
from migrate_config import backup_config, main, migrate_config
...
class TestMigrateConfig:
    ...
    def test_v1_11_with_slack_preserves_slack(self):
        """v1.11 config에 slack 있으면 보존, telegram/discord 추가."""
    ...
    def test_already_v2_returns_message(self):
        """이미 v2.0이면 '이미 v2.0' 메시지 반환."""
        config = {"version": "2.0", "notifications": {}}
        migrated, changes = migrate_config(config)

        assert migrated is config  # 동일 객체 반환 (deep copy 안 함)
        assert len(changes) == 1
        assert "이미 v2.0" in changes[0]
    ...
    def test_no_version_field_treated_as_empty(self):
        ...
        assert migrated["version"] == "2.0"
        version_change = [c for c in changes if "version" in c][0]
...
class TestMain:
    ...
    def test_dry_run_already_v2_shows_no_changes(self, tmp_path, monkeypatch, capsys):
        """이미 v2.0인 config에 --dry-run 실행 시 '변경 사항 없음' 표시."""
        config_path = tmp_path / "config.json"
        self._write_config(str(config_path), {"version": "2.0", "notifications": {}})
    ...
    def test_normal_run_already_v2_skips_backup(self, tmp_path, monkeypatch, capsys):
        """이미 v2.0이면 백업/파일쓰기 없이 즉시 종료."""
        config_path = tmp_path / "config.json"
        v2_config = {"version": "2.0", "notifications": {"slack": {"enabled": True, "webhook_url": "url"}}}
        self._write_config(str(config_path), v2_config)

        monkeypatch.setattr(sys, "argv", ["migrate-config.py", "--config", str(config_path)])
        exit_code = main()
        assert exit_code == 0

        # 이미 v2.0이면 백업 생성하지 않음
        assert not os.path.exists(str(config_path) + ".bak")
        captured = capsys.readouterr()
        assert "이미 v2.0" in captured.out
